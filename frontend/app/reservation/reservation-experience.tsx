...
interface Props {
    id: string
    reservation: 'BUYNOW' | 'ADDTOCART' | 'GIFT' | null
}
...
const ReservationExperience = ({ id, reservation }: Props) => {
    const [experience, setExperience] = useState<ExperienceTypes | null>(null)
    ...
    const handleSubmit = () => {
        ...
        if (reservation === 'GIFT') {
            ...
            const existingCart = existingCartJSON
        ...
        setKey()
        resetContext()
        experience?.extras.forEach((extra) => (extra.quantity = 0))

        if (reservation === 'BUYNOW') {
            router.push('/cart')
        } else if (reservation === 'ADDTOCART') {
            window.scrollTo(0, 0)
        }
    }
        ...
        setExtras((prevExtras) => {
            const updatedExtras = prevExtras.map((extra) =>
                extra.id === newExtra.id
                    ? { ...extra, quantity: newExtra.quantity }
                    : extra
            )
    ...
    const adjustedPrices: any = getExperiencePrices(
        selectedDuration,
        adults + child,
        experience?.options || []
    )
    ...
    const calculateTotal = (prices: any) => {
        ...
        return {
            USD:
                prices.adult.USD * adults +
                prices.child.USD * child +
                extrasTotalUSD,
            LKR:
                prices.adult.LKR * adults +
                prices.child.LKR * child +
                extrasTotalLKR,
        }
    }

    return (
        <div className="h-full w-full bg-blue-50 p-6">
            {experience === null ? (
                                ...
                                <Card className="flex w-56 flex-col items-center px-5 py-8 shadow-lg shadow-blue-200 md:w-64">
                                    ...
                                    <p className="text-lg font-medium">
                                        {local ? (
                                            <>
                                                LKR{' '}
                                                {adjustedPrices?.adult.LKR *
                                                    adults}
                                            </>
                                        ) : (
                                            <>
                                                USD $
                                                {adjustedPrices?.adult.USD *
                                                    adults}
                                            </>
                                        )}
                                    </p>
