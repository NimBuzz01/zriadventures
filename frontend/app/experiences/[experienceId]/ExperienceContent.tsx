...
const ExperienceContent = ({ experienceId }: { experienceId: string }) => {
  ...
  const [nearbyExperiences, setNearbyExperiences] = useState<ExperienceTypes[]>(
    []
  );
  ...
  useEffect(() => {
    if (experience) {
      const nearbyExps = experiences.filter(
        (exp: ExperienceTypes) => exp.location.id === experience.location.id
      );
    ...
    >
      <ExperienceProvider>
        {experience ? (
          <>
            ...
            {nearbyExperiences.length > 0 && (
              <div className="relative flex flex-col items-center justify-center gap-10 bg-gradient-to-r from-cyan-100 to-blue-50 px-2 py-16">
                <MainHeader
                  subtitle={EXPERIENCE_NEARBY_EXPERIENCE_SUB_TITLE}
                ...
                <div className="z-10 w-full sm:w-[90%]">
                  <Carousel
                    ...
                    dots
                  >
                    {nearbyExperiences.map((experience: ExperienceTypes) => (
                      <ExperienceCard
                        key={experience.id}
          ...
          <div className="h-screen w-full">
            <NotFoundLabel text={"Sorry! The experience does not exist."} />
          </div>
        )}
      </ExperienceProvider>
    </Suspense>
