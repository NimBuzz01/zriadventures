...
import { ExperienceDuration } from "@/types/experienceTypes";
...
const PricingOptions: React.FC<Props> = ({ options }) => {
  const { selectedDuration, setSelectedDuration } = useExperienceContext();
  ...
  const getSelectedPaxForDuration = (duration: ExperienceDuration) => {
  ...
  const handleDurationChange = (duration: ExperienceDuration) => {
  ...
  return (
    <div>
      ...
      <div className="mb-4 mt-5">
        <p className="mb-1 font-medium text-gray-600">Available Duration</p>
        <div className="flex flex-wrap gap-2">
          {options.map((option) => (
            ...
            >
              <input
                type="radio"
                value={option.duration.type + "-" + option.duration.amount}
                checked={
                  selectedDuration?.type === option.duration.type &&
                  selectedDuration?.amount === option.duration.amount
                }
                onChange={() => handleDurationChange(option.duration)}
              ...
              ?.paxRates.map((rate: any) => (
                ...
                >
                  <input
                    ...
                    onChange={() => handlePaxChange(rate.minPax)}
                    className="peer mr-2 hidden"
                  />
                  {rate.minPax}
                  {rate.maxPax !== null ? " - " + rate.maxPax : " - more"} Pax
                </label>
              ))}
          </div>
        </div>
