...
import { getPrices, useExperienceContext } from "@/contexts/experience-context";
...
const Reservation = ({ experience, addType }: Props) => {
  ...
  const adjustedPrices = getPrices(
    selectedDuration,
    adults + child,
    experience.options
  );
      ...
      <Card className="w-full max-w-[1000px] bg-blue-50 p-6">
        {success ? (
          <div className="flex h-[600px] flex-col items-center justify-center">
            ...
            <MainButton href="/experiences" text="Browse more Experiences" />
          </div>
        ) : (
          <>
            ...
            <Card className="mb-4 mt-5 flex flex-col items-center justify-center py-10 ">
              ...
              <div className="flex flex-wrap gap-2">
                {experience.options.map((option) => (
                  <label
                    key={option.duration.type + "-" + option.duration.amount}
                    className={`inline-flex items-center rounded-md border-2 px-3 py-2 text-sm font-medium uppercase ${
                      selectedDuration?.type === option.duration.type &&
                      selectedDuration?.amount === option.duration.amount
                  ...
                  >
                    <input
                      type="radio"
                      value={
                        option.duration.type + "-" + option.duration.amount
                      }
                      checked={
                        selectedDuration?.type === option.duration.type &&
                        selectedDuration?.amount === option.duration.amount
            ...
            <div className="mb-10 flex flex-wrap justify-center gap-6 sm:gap-10">
              <Card className="flex w-56 flex-col items-center px-5 py-8 shadow-lg shadow-blue-200 md:w-64">
                ...
                />
                {inputErrors.pax && (
                  <p className="mt-1 text-xs text-red-500">{inputErrors.pax}</p>
                )}
                <p className="mt-4 text-xs sm:text-sm">Total</p>
                <p className="text-lg font-medium">
                  {local ? (
                    <>LKR {adjustedPrices?.adult.LKR * adults}</>
                  ) : (
                    <>USD ${adjustedPrices?.adult.USD * adults}</>
                  )}
                </p>
              </Card>
              <Card className="flex w-56 flex-col items-center px-5 py-8 shadow-lg shadow-blue-200 md:w-64">
                ...
                <p className="text-lg font-medium">
                  {local ? (
                    <>
                      LKR {adjustedPrices && adjustedPrices?.child.LKR * child}
                    </>
                  ) : (
                    <>
                      USD ${adjustedPrices && adjustedPrices?.child.USD * child}
                    </>
                  )}
                </p>
              </Card>
            ...
            <div className="flex items-center">
              ...
              <span className="mx-4 flex-shrink text-gray-400">Extras</span>
              <div className="flex-grow border-t border-gray-400"></div>
            </div>
            {experience.extras && (
              <div className="mb-10 mt-6 flex flex-wrap justify-center gap-6 text-center md:gap-10">
                {experience.extras.map((extra) => (
                  ...
                  >
                    ...
                    <p className="text-lg font-medium">
                      {local ? (
                        <>LKR {extra.cost.LKR * extra.quantity}</>
                      ) : (
                        <>USD ${extra.cost.USD * extra.quantity}</>
                      )}
                    </p>
                  </Card>
                ))}
            ...
            <div className="flex flex-col lg:flex-row">
              {addType !== "gift" && (
                <div className="flex flex-col justify-center">
                  ...
                  {inputErrors.date && (
                    <p className="text-xs text-red-500">{inputErrors.date}</p>
                  )}
            ...
            <div className="flex flex-col items-start gap-4 lg:flex-row lg:items-center">
                ...
                <p className="text-xs text-gray-600 sm:text-base">
                  ...
                  <span className="ml-1 text-2xl font-semibold text-gray-800">
                    {selectedDuration && adjustedPrices && (
                      <>
                        {local ? (
                          <>LKR {calculateTotal(adjustedPrices).LKR}</>
                        ) : (
                          <>USD ${calculateTotal(adjustedPrices).USD}</>
                        )}
                      </>
                    )}
                  </span>
              ...
              >
                {addType === "buy" ? "Proceed to Checkout" : "Add to Cart"}
              </button>
            </div>
          </>
        )}
