...
import { convertLKRtoUSD, sendEmail } from "@/lib/utils/func";
...
const Overview = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      if (vendor === "mintpay" && orderId && mintpayStatus !== "Success") {
      ...
      try {
        if (orderId) {
          ...
          const updatedOrder = {
            ...
            totalAmount,
            currency,
            status: paymentVendor === "BANK" ? "SUCCESS" : paymentStatus,
          ...
          if (paymentStatus === "SUCCESS" || paymentVendor === "BANK") {
          ...
          if (orderEmail !== "true") {
            ...
            if (paymentStatus === "SUCCESS" && paymentVendor !== "BANK") {
              ReactGA.gtag("event", "purchase", {
                ...
                value: amount,
                currency: "USD",
                items: getSortedItems(tempOrder.items),
              });
              sendEmailReceipt(
                tempOrder.firstName,
                tempOrder.orderId,
            ...
            const vouchers: any[] = getFilteredItems(items, "VOUCHER");

            if (
              vouchers.length > 0 &&
              (paymentStatus === "SUCCESS" ||
                (paymentStatus === "PENDING" && paymentVendor === "BANK"))
            ) {
              vouchers.map(async (voucher) => {
                const voucherData = {
                  data: {
                    couponCode: voucher.item.voucher.couponId[0],
                    status: paymentVendor === "BANK" ? "UNPAID" : "AVAILABLE",
                    expiryDate: voucher.item.voucher.expiryDate,
                    type: voucher.item.voucher.voucherType.toUpperCase(),
                    ...(voucher.item.voucher.voucherType === "Cash"
                      ? {
                          cash: {
                            amount: voucher.item.voucher.voucherAmount.amount,
                            currency:
                              voucher.item.voucher.voucherAmount.currency,
                          },
                        }
                      : voucher.item.voucher.voucherType === "Experience"
                      ? {
                          experience: [
                            {
                              name: voucher.item.voucher.voucherExperience.item
                                .name,
                              adults:
                                voucher.item.voucher.voucherExperience.item
                                  .adults,
                              children:
                                voucher.item.voucher.voucherExperience.item
                                  .children,
                              extras:
                                voucher.item.voucher.voucherExperience.item.extras.map(
                                  (extra: any) => ({
                                    name: extra.name,
                ...
                if (voucher.item.info.recipientsEmail !== "") {
                  const receipt = createGiftVoucherTemplate(voucher);
                  sendEmail(
  ...
  return (
    <div className="w-full px-4 py-10 sm:p-16">
      {loaded ? (
        <>
          <PaymentStatusLabel
            status={order.status as "SUCCESS" | "FAILED" | "PENDING"}
          />
          {order.status === "SUCCESS" ? (
            <>
              <p className="mt-4 text-center text-gray-500">
                Your reservation is confirmed. You will receive a receipt to
                your email and our team will get back to you shortly!
              </p>
              <div className="mt-10 font-medium">
                ...
                <div className="mb-2 flex flex-col sm:flex-row sm:justify-between">
                  <p>Contact Number:</p>
                  <p className="font-normal">{order.contactNumber}</p>
                </div>
                <div className="mb-2 flex flex-col sm:flex-row sm:justify-between">
                  <p>Delivery Address:</p>
                  <p className="font-normal">{order.deliveryAddress}</p>
                </div>
              </div>
              <div className="mt-10 font-medium">
                ...
                <div className="flex items-end justify-between">
                  <p>Amount Paid</p>
                  <p className="text-lg font-normal">
                    {order.currency} {order.amountPaid.toFixed(2)}
                  </p>
                </div>
            ...
            <div className="mt-10 flex flex-col items-center justify-center gap-10 text-center">
              <p className="text-lg ">
                Something went wrong! Please checkout again or contact us for
                manual reservations. Sorry for the inconvenience.
              </p>
