    ...
    <>
      <div className="flex w-full flex-col justify-center gap-16 bg-gray-50 px-4 py-20 sm:px-10 lg:flex-row">
        <div className="w-full lg:w-1/2 ">
          <div className="mb-5">
            <Breadcrumbs first="Events" firstHref="/events" last={event.name} />
          </div>
          ...
          <div className="mb-10 flex flex-wrap items-center justify-center gap-12 sm:gap-24">
            ...
            <div>
              ...
              <p className="text-lg font-medium text-blue-950 sm:text-xl md:text-2xl">
                {event.duration.amount + " " + event.duration.type}
              </p>
            </div>
            <div>
              <p className="text-sm font-medium uppercase text-gray-500">
          ...
          <div className="my-6">
            <p className="mb-2 text-xl font-semibold">Image Gallery</p>
          ...
          <CustomAccordion title="Additional Information">
            {event.requirements.map((req, index: number) => (
              <div key={index} className="my-6">
                ...
                <p className="text-sm sm:text-base">{req.description}</p>
              </div>
            ))}
          </CustomAccordion>
          <CustomAccordion title="Frequently asked questions">
            {event.faq.map((f, index) => (
              <div key={index} className="my-6">
                ...
                <p className="text-sm sm:text-base">{f.answer}</p>
              </div>
            ))}
          </CustomAccordion>
          ...
          <CustomAccordion title="Terms and Conditions">
            <ul className="list-disc">
              {event.terms.map((e, index: number) => (
                <li key={index} className="my-4 text-sm sm:text-base">
                  <Markdown>{e}</Markdown>
                </li>
              ))}
            </ul>
          ...
          {new Date(event.startDate) >= new Date() && (
            <div>
              <p className="text-3xl font-semibold">
                {local ? <>LKR {event.cost.LKR}</> : <>USD ${event.cost.USD}</>}{" "}
                <span className="text-sm font-medium uppercase">per Adult</span>
              </p>
              <p className="text-lg font-semibold">
                {local ? <>LKR {event.cost.LKR}</> : <>USD ${event.cost.USD}</>}{" "}
                <span className="text-sm font-medium uppercase">per Child</span>
              </p>
            </div>
          )}
          <Dialog>
            <DialogTrigger
              asChild
              disabled={new Date(event.startDate) < new Date() ? true : false}
            >
              <button
                disabled={new Date(event.startDate) < new Date() ? true : false}
            ...
            <DialogContent className="z-[1001] h-[100dvh] overflow-y-auto scroll-smooth scrollbar-thin scrollbar-track-gray-100 scrollbar-thumb-gray-800 sm:h-[90dvh]  sm:max-w-[1000px]">
              <div
                className={`w-full flex-col  px-4 py-10 sm:px-10 ${
                  success && "flex h-full items-center justify-center gap-10"
                }`}
              >
                {success ? (
                  <>
                ...
                <div className="mt-10 text-gray-500">
                  ...
                  <p className="mb-4 text-sm">
                    Go Through Checkout to Pay for the Event. It can be either
                    through bank or the available Payment Vendors.
                  </p>
                  <p className="font-medium">Your reservation is confirmed!</p>
                  <p className="mb-4 text-sm">
                    Once your payment is verified, You will receive reservation
