...
const RentalCartCard = ({ item, onDelete }: Props) => {
  ...
  const handleDeleteClick = () => {
    onDelete(rentalItem.id); // Call the onDelete callback with the item id
    setKey();
  };
          ...
          <p className="text-lg font-medium md:text-xl">
            {local ? (
              <>
                LKR{" "}
                {(
                  rentalItem.quantity * rentalItem.selectedOption.cost.LKR
                ).toFixed(2)}
              </>
            ) : (
              <>
                USD $
                {(
                  rentalItem.quantity * rentalItem.selectedOption.cost.USD
                ).toFixed(2)}
              </>
            )}
          </p>
        ...
        <div className="flex flex-col w-full md:flex-row">
          <div className="w-full md:w-1/2">
            ...
            <div className="flex flex-col mb-2">
              ...
              <span className="flex items-center gap-2">
                <SlCalender />
                {formatDate(rentalItem.endDate.toString().split("T")[0])}
              </span>
            </div>
            <div className="flex flex-col mb-2">
