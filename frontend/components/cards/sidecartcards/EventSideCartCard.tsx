      ...
      <div className="mx-2 my-2 flex flex-grow flex-col">
        ...
        <div className="mt-auto flex items-center justify-between">
          <p className="text-sm text-gray-600">
            Pax: <span className="font-semibold">{item.item.info.pax}</span>
          </p>
          <p className="font-medium text-blue-900">
            {local ? (
              <>
                LKR {(item.item.info.pax * item.item.event.cost.LKR).toFixed(2)}
              </>
            ) : (
              <>
                USD $
                {(item.item.info.pax * item.item.event.cost.USD).toFixed(2)}
              </>
            )}
          </p>
        </div>
