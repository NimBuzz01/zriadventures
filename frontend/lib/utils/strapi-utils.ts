...
import { incrementOrderId } from "../utils";
...
export async function validateVoucherId(couponCode: string, local: boolean) {
  const apiUrl = `${baseURL}/api/vouchers?filters[couponCode][$eq]=${couponCode}&populate=deep`;
...
export const setCouponStatus = async (
  ...
  const apiUrl = `${baseURL}/api/vouchers/${id}`;
