    ...
    const mappedExperiences: ExperienceTypes[] = (
        ...
        return {
            ...
            location: {
                ...
                coordinates: {
                    latitude:
                        data.location.data.attributes.coordinates.latitude ||
                        '',
                    longitude:
                        data.location.data.attributes.coordinates.longitude ||
                        '',
                },
                ...
                googleMapsLink:
                    data.location.data.attributes.googleMapsLink || '',
            },
            options: data.options.map((option: any) => ({
                ...
                childCostReduction: {
                    USD: option.childCostReduction
                        ? option.childCostReduction.USD
                        : 0,
                    LKR: option.childCostReduction
                        ? option.childCostReduction.LKR
                        : 0,
                },
            })),
            extras:
            ...
            images: data.images.data
                ? data.images.data.map((image: any) => ({
                      src:
                          image.attributes.url.replace(/\.[^.]+$/, '.webp') ||
                          '',
            ...
            terms: data.terms_and_condition.data.attributes.terms.split('\n'),
            suitedVoucher: data.voucher_templates.data.map((voucher: any) => ({
                ...
                alt:
                    voucher.attributes.image.data.attributes.alternativeText ||
                    '',
            })),
...
export async function getExperienceLocationData(): Promise<
    ExperienceLocation[]
> {
    const response = await getExperienceLocations()
    const mappedLocations: ExperienceLocation[] = (
        response as AxiosResponse<any>
    ).data.map((res: any) => {
        const data = res.attributes
        return {
            ...
            image: {
                src: data.image.data.attributes.url.replace(
                    /\.[^.]+$/,
                    '.webp'
                ),
